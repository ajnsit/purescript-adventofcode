module Year2020.Day4.Main where
...
import Util.Parse as Util

main :: Effect Unit
main = do
  passports <- readInputParagraphs "src/Year2020/Day4/input"
  -- passports <- pure (A.take 2 passports)
  -- log $ show $ passports
  -- log $ show $ map strPassport passports
  -- log $ show $ map parsePassport passports
  -- log $ show $ toMap (Just M.empty) passports
  -- let reg = eToMaybe $ SR.regex "\\d+(cm|in)" regexFlags
  -- log $ show $ reg
  log $ "Passports with all fields present: " <> (show $ A.length $ A.mapMaybe parsePassport1 passports)
  log $ "Passports with all valid fields: " <> (show $ A.length $ A.mapMaybe parsePassport passports)
