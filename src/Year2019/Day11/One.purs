module Year2019.Day11.One where
...
main = do
  ...
  log $ show $ countPaintedPixels $ fromMaybe FO.empty $ map _.state.canvas $ runProgram 0 program
...
countPaintedPixels :: Canvas -> Int
countPaintedPixels c = sum $ map countRowPixels (indices c)
  where
    countRowPixels i = A.length $ indices $ extractRow c i

-- Unused. And doesn't work (doesn't render the unpainted tiles)
showCanvas :: Canvas -> String
showCanvas c = S.joinWith "\n" $ map (showRow <<< extractRow c) (indices c)

extractRow :: forall a. Show a => Canvas -> a -> FO.Object Int
extractRow x i = fromMaybe FO.empty $ FO.lookup (show i) x

showRow :: FO.Object Int -> String
showRow x = SC.fromCharArray $ map dispPix $ A.catMaybes $ map (\i -> FO.lookup (show i) x) (indices x)

indices :: forall a. FO.Object a -> Array Int
indices = A.sort <<< A.catMaybes <<< map parseInt10 <<< FO.keys
